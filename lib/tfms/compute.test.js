...
describe("computeProjectedFlights", () => {
  ...
  it("filters invalid flights and returns projected summary fields", () => {
    ...
    for (const flight of flights) {
      ...
      expect(flight).toHaveProperty("proj5Specialty");
      expect(flight).toHaveProperty("proj10Sector");
      expect(flight).not.toHaveProperty("callsign");
...
describe("summary builders", () => {
  ...
  it("buildSpecialtySummary excludes TRACON altitude flights", () => {
    const result = buildSpecialtySummary(projections, ["N", "S"]);
    expect(result).toEqual([
      { specialty: "N", now: 1, p5: 1, p10: 0, p20: 0 },
      { specialty: "S", now: 0, p5: 0, p10: 1, p20: 1 },
    ]);
  });
  ...
  it("buildSplitSummary counts by split sectors", () => {
    ...
    expect(result).toEqual([
      { name: "northSplit", now: 1, p5: 1, p10: 0, p20: 0 },
      { name: "southSplit", now: 0, p5: 0, p10: 1, p20: 1 },
    ]);
  });
  ...
  it("counts projected buckets using projected altitude thresholds", () => {
    const altitudeAware = [
      {
        ...
        proj20Sector: "11",
        proj5Altitude: 9500,
        proj10Altitude: 10000,
        proj20Altitude: 11500,
      },
    ];
    ...
    expect(specialty).toEqual([{ specialty: "N", now: 0, p5: 0, p10: 1, p20: 1 }]);
    expect(split).toEqual([{ name: "northSplit", now: 0, p5: 0, p10: 1, p20: 1 }]);
  });
});
