...
const OPERATIONAL_GROUNDSPEED_KTS = 40;
const OPERATIONAL_ALTITUDE_FT = 2_000;
const NEAR_PERIMETER_DISTANCE_NM = 90;
const PROJECT_TO_PERIMETER_NM = 25;
const INBOUND_HEADING_TOLERANCE_DEG = 45;
const ENHANCED_MODEL_MARGIN_NM = 180;
const PROJECTION_WINDOWS_MIN = [10, 20, 30];
...
export function classifyPosition(lat, lon, altitude, sectorIndex, excludeZhu = true) {
  ...
  for (const feature of sectorIndex.features) {
    ...
    const isInsidePolygon = pointInPolygon([lon, lat], feature.polygon);
    if (!isInsidePolygon) {
      continue;
    }
    polygonMatches.push(feature);
    if (altitude >= feature.floor && altitude < feature.ceiling) {
      return feature.properties;
    }
  }
  if (polygonMatches.length > 0) {
    // Altitude-gap fallback: keep specialty/sector attribution when lateral coverage exists
    // but local vertical band definitions do not include the exact altitude.
    let fallback = null;
    let minAltitudeDistance = Infinity;
    for (const feature of polygonMatches) {
      const floor = Number.isFinite(feature.floor) ? feature.floor : -99_999;
      const ceiling = Number.isFinite(feature.ceiling) ? feature.ceiling : 99_999;
      let distance = 0;
      if (altitude < floor) {
        distance = floor - altitude;
      } else if (altitude >= ceiling) {
        distance = altitude - ceiling;
      }
      if (distance < minAltitudeDistance) {
        minAltitudeDistance = distance;
        fallback = feature;
      }
    }
...
export function passesOperationalGate(altitudeFt, groundspeedKts) {
  return Number(groundspeedKts || 0) > OPERATIONAL_GROUNDSPEED_KTS || Number(altitudeFt || 0) > OPERATIONAL_ALTITUDE_FT;
}
...
export function computeProjectedFlights(vatsim, sectorIndex, motionByCallsign = null) {
  const flights = (vatsim.pilots || [])
    ...
    .filter((flight) => passesOperationalGate(coerceAltitude(flight.altitude), Number(flight.groundspeed || 0)));
  ...
  return flights.map((flight) => {
    ...
    const p10 = projectPilotState(flight, motionState, PROJECTION_WINDOWS_MIN[0]);
    const p20 = projectPilotState(flight, motionState, PROJECTION_WINDOWS_MIN[1]);
    const p30 = projectPilotState(flight, motionState, PROJECTION_WINDOWS_MIN[2]);
    ...
    const s30 = classifyPosition(p30.latitude, p30.longitude, p30.altitudeFt, sectorIndex, false);

    return {
      mapId: String(flight.cid || callsign || `${flight.latitude}:${flight.longitude}`),
      callsign: String(flight.callsign || ""),
      departure: normalizeAirportCode(flight?.flight_plan?.departure),
      arrival: normalizeAirportCode(flight?.flight_plan?.arrival),
      isEnteringZhu: Boolean(flight.isEnteringZhu),
      heading: Number.isFinite(Number(flight.heading)) ? Number(flight.heading) : 0,
      latitude: flight.latitude,
      longitude: flight.longitude,
      proj10Latitude: p10.latitude,
      proj10Longitude: p10.longitude,
      proj20Latitude: p20.latitude,
      proj20Longitude: p20.longitude,
      proj30Latitude: p30.latitude,
      proj30Longitude: p30.longitude,
      altitude,
      groundspeed: Number(flight.groundspeed || 0),
      proj10Altitude: p10.altitudeFt,
      proj20Altitude: p20.altitudeFt,
      proj30Altitude: p30.altitudeFt,
      ...
      proj30Specialty: s30?.specialty || null,
      proj10Sector: s10?.sector || null,
      proj20Sector: s20?.sector || null,
      proj30Sector: s30?.sector || null,
    };
  });
}
...
export function buildSpecialtySummary(projections, specialties) {
  ...
  const p30 = Object.fromEntries((specialties || []).map((name) => [name, 0]));
  ...
  for (const flight of projections || []) {
    ...
    const p30Alt = coerceAltitude(
      flight.proj30Altitude !== undefined ? flight.proj30Altitude : flight.altitude,
    );

    if (passesOperationalGate(nowAlt, groundspeed) && flight.specialty) {
      now[flight.specialty] = (now[flight.specialty] || 0) + 1;
    }
    if (passesOperationalGate(p10Alt, groundspeed) && flight.proj10Specialty) {
      p10[flight.proj10Specialty] = (p10[flight.proj10Specialty] || 0) + 1;
    }
    if (passesOperationalGate(p20Alt, groundspeed) && flight.proj20Specialty) {
      p20[flight.proj20Specialty] = (p20[flight.proj20Specialty] || 0) + 1;
    }
    if (passesOperationalGate(p30Alt, groundspeed) && flight.proj30Specialty) {
      p30[flight.proj30Specialty] = (p30[flight.proj30Specialty] || 0) + 1;
    }
  }
...
export function buildSplitSummaryFromIndex(projections, splitIndex) {
  ...
  return splitIndex.map((split) => {
    ...
    for (const flight of projections || []) {
      ...
      if (passesOperationalGate(nowAlt, groundspeed) && flight.sector && sectors.has(flight.sector)) now += 1;
      if (passesOperationalGate(p10Alt, groundspeed) && flight.proj10Sector && sectors.has(flight.proj10Sector)) p10 += 1;
      if (passesOperationalGate(p20Alt, groundspeed) && flight.proj20Sector && sectors.has(flight.proj20Sector)) p20 += 1;
      if (passesOperationalGate(p30Alt, groundspeed) && flight.proj30Sector && sectors.has(flight.proj30Sector)) p30 += 1;
    }

    return { name: split.name, now, p10, p20, p30 };
  });
}
